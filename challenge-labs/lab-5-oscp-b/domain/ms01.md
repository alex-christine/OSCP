...
At this point I check the domain account lockout policy in case I need to do some password spraying:

```
net accounts /domain
```

<figure><img src="../../../.gitbook/assets/OSB-Domain-LockoutPolicy.png" alt=""><figcaption><p>Lockout policy for domain</p></figcaption></figure>

There is no lockout which is very helpful.
...
Once the `ligolo-ng` proxy is set up, I can enumerate the internal network with Nmap:

{% code overflow="wrap" %}
```bash
nmap -sT -Pn -A -p- -o internal_tcp_conn_all.nmap ms02.oscp.exam dc01.oscp.exam
```
{% endcode %}
